...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setError(err.message === 'Unauthorized admin request.' ? 'Invalid API key. Please try again.' : err.message);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="mb-8 flex flex-col items-center gap-3">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-cyan-400/30 bg-cyan-400/10 shadow-lg shadow-cyan-500/10">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 text-cyan-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M17.25 6.75L22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3l-4.5 16.5" />
            </svg>
          </div>
          <div className="text-center">
            <h1 className="font-display text-2xl font-bold tracking-tight text-white">CodeShare</h1>
            <p className="mt-1 text-sm text-slate-400">Secure read-only code sharing platform</p>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-white/10 bg-slate-950/80 p-8 shadow-2xl shadow-black/40 backdrop-blur-2xl">
          ...
          <p className="mb-6 text-sm text-slate-400">Enter your API key to access the admin panel.</p>

          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              <label htmlFor="api-key-input" className="text-xs font-medium uppercase tracking-widest text-slate-400">
                API Key
              </label>
        ...
        <p className="mt-6 text-center text-xs text-slate-600">
          ...
          <code className="rounded bg-white/5 px-1 py-0.5 text-slate-400">/view/project-name/token</code>
        </p>
      </div>
    </div>
  );
